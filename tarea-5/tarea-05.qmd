...
No es buena porque un modelo trivial que prediga "no compra" para cualquier entrada tendría una tasa de predicción del $93.91\%$, o sea nuestro modelo no supera al modelo trivial. 

### Regresión logística regularizada

Usaremos regresión logística regularizada (veremos más en la próxima clase,
por el momento considera que este método produce para cada caso probabilidad
de que un cliente particular compre la póliza).
...
head(pred_probas |> select(.pred_Yes, .pred_No, Purchase))
```
...
Tenemos únicamente 3 predicciones de candidatos a comprar, mientras que tenemos 1744 candidatos a no hacerlo.

Considero que no tiene sentido que con estos datos encontremos probabilidades muy grandes ya que la clase de No compra está subrepresentada, entonces el modelo está aprendiendo a decir "Sí compra" y no está generalizando los "No compra", por lo que la probabilidad de que alguien compre uno de los seguros es muy poca tomando en cuenta los datos que tenemos. 

**NOTA: ** Es preferible no cortar en 0.5 ya que únicamente funciona si las clases están bien separadas. 
...
realmente es compradorde este seguro? (este último número se llama *precisión*).

Se encontraron 274 candidatos totales, lo que equivale a $15.68\%$ de la muestra total. 

El porcentaje de candidatos que realmente son compradores de los seguros es 
de $46/(228+46) \approx 16.79\%$

Si escoges al azar de la muestra de prueba el mismo número de candidatos, 
¿qué porcentaje de compradores esperas encontrar en este grupo escogido al azar? 
Compara esto con lo que obtuviste usando el modelo. 
¿Qué tan superior es tu modelo en escoger comparado con escoger clientes al azar 
para intentar venderles el seguro?

```{r}
set.seed(8)
counts <- sample_n(caravan_prueba, size=274) |> 
  group_by(Purchase) |> count() |>
  pivot_wider(names_from = Purchase, values_from = n)
print(paste0("Proporcion No: ", counts$No / sum(counts)))
print(paste0("Proporcion Si: ", counts$Yes / sum(counts)))
print(paste0("Cantidad de posibles compradores: ", counts$Yes))
```
Al escoger al azar esperamos encontrar un porcentaje cercano a la proporción poblacional de candidatos para comprar el seguro, que es aproximadamente del $6\%$.

El modelo encuentra más candidatos a comprar el seguro, pero se equivoca muchísimo. 
...
De todos los compradores verdaderos, se clasificaron correctamente $46\%$. Se escaparon $54\%$ de posibles compradores. 

Este modelo es un mal clasificador ya que no estamos capturando ni la mitad de todos los posibles compradores ni con el corte realizado, por lo que posiblemente la campaña no tenga el alcance esperado. 
...
```{r}
pred_probas <- pred_probas |> 
  mutate(tipo = ifelse(.pred_Yes > 0.3, "candidato", "no_candidato"))
...
Esperaríamos que conforme aumentamos el punto de corte disminuya la cantidad de usuarios clasificados como sí candidatos ya que somos menos tolerantes a los errores, por lo que también aumentaría la precisión, sin embargo disminuye el recall. Si bajamos el punto de corte, esperaríamos que aumente la cantidad de usuarios clasificados como sí candidatos, la precisión baja pero el recall sube. 
...
```
El punto de corte de probabilidad más alta es de $57\%$, con este valor tendríamos una cobertura de 0 clientes. En la gráfica se observa en la parte superior derecha. 

El punto de corte de probabilidad más bajo es de $3\%$, con este valor tendríamos una cobertura de 0 clientes. En la gráfica se observa en la parte inferior izquierda. 

Deberíamos tomar al rededor de 500 clientes para capturar al $60\%$ de los posibles compradores, y esto de lograría con un punto de corte en probabilidad de $6.8\%$.
